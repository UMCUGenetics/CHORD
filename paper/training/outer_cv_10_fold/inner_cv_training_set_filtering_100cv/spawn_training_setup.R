...
#========= Exec =========#
set.seed(1)
training_set_split <- createCvTrainTestSets(training_set)

counter <- 0
for(i in training_set_split){
  counter <- counter+1

  root_dir <- paste0(base_dir,'/HMF_DR010_DR047/scripts/chord_training/main/output/60_CV_snvContext_svContext_allSigsRel_noSuppEvidence_noBoruta/')
  fold_name <- formatC(counter, width=nchar(length(training_set_split)), format="d", flag="0")
  parent_dir <- paste0(root_dir,'/fold_',fold_name,'/')
  if(!dir.exists(parent_dir)){
    dir.create(parent_dir)
  }

  message(paste0('\n## Spawning setup for CV fold: ',fold_name))
  spawnTrainingSetups(
    training_set=i$train,
    test_set=i$test,
    seeds=1:100, parent.dir=parent_dir
  )
  #if(counter==1){ break }
}

...
calcUniqVariantFreq <- function(diplotypes, mode='germ'){
   ...
   pb <- txtProgressBar(max=nrow(diplotypes_ss), style=3)
   counter <- 0
   diplotypes_ss_string <- apply(diplotypes_ss,1,function(i){
      counter <<- counter + 1
   ...
   df$freq_prop <- df$freq/length(df$freq)
...
path_pon_germ <- paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/pon_germ.txt')

if(file.exists(path_pon_germ)){
   pon_germ <- read.delim(path_pon_germ)
   ...
   write.table(pon_germ,path_pon_germ,sep='\t', quote=F, row.names=F)
}
...
## Export
pdf(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/pon_germ.pdf'), 5,5)
grid.draw(pon_plot)
dev.off()
...
path_diplotypes_filt <- paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/diplotypes_filt.txt.gz')

if(file.exists(path_diplotypes_filt)){
   diplotypes_filt <- read.delim(path_diplotypes_filt)
} else {
...
fisherTestCustom <- function(
   diplotypes=diplotypes_filt, genes=NULL, 
   ...
   chord.pred=pred, chord.cutoff=0.5, 
   fisher.cutoff=0.1, use.pvalue=F,
   min.freq.p=5, 
   ...
   #diplotypes_ss <- diplotypes[,c('sample','hgnc_symbol','diplotype_origin','a1.max_score','a2.max_score')]
   diplotypes_ss <- diplotypes
   diplotypes_ss <- merge(diplotypes_ss, chord.pred[,c('sample','hrd')], by='sample', all.x=T)
   
   # diplotypes_ss$is_def <- with(diplotypes_ss, {
   #    unlist(Map(function(diplotype_origin, a1.max_score, a2.max_score){
   #       cutoffs <- min.allele.scores[[diplotype_origin]]
   #       a1.max_score >= cutoffs[1] & a2.max_score >= cutoffs[2]
   #    }, diplotype_origin, a1.max_score, a2.max_score, USE.NAMES=F))
   # })
   #diplotypes_ss <- detIsDef(diplotypes_ss, min.allele.scores)
   ...
   fisherTestByGene <- function(hgnc_symbol){
      #hgnc_symbol='BRCA2'
      
      df <- diplotypes_ss[diplotypes_ss$hgnc_symbol==hgnc_symbol,]
...
path_hr_gene_likelihood <- paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/hr_gene_likelihood.txt')

if(file.exists(path_hr_gene_likelihood)){
   hr_gene_likelihood <- read.delim(path_hr_gene_likelihood)
} else {
   hr_gene_likelihood <- fisherTestCustom(diplotypes_filt)
...
plotFisherTest <- function(
   df, fisher.cutoff=0.1, min.freq.p=5,
...
plotFisherTestWrapper(
   subset(hr_gene_likelihood[order(hr_gene_likelihood$keep),], keep!=-1),
   paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/hr_gene_likelihood_fisher_filt2.pdf')
)

plotFisherTestWrapper(
   hr_gene_likelihood[order(hr_gene_likelihood$keep),],
   paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/hr_gene_likelihood_fisher2.pdf')
)
...
mkCnvCooccMat <- function(diplotypes, gene1, gene2){
   ...
   l <- lapply(genes, function(i){
      df <- diplotypes[diplotypes$hgnc_symbol==i,c('sample','a1')]
      colnames(df)[2] <- i
      return(df)
   })
   ...
   cn_effects <- c('full_gene_loss','trunc','loh')
   #cn_effects <- c('full_gene_loss','trunc')
...
path_cnv_coocc <- paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/cnv_coocc.rds')
...
pdf(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/cnv_coocc_fisher2.pdf'), 10, 6)
...
#--------- Fisher table ---------#
write.table(
   hr_gene_likelihood, paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/hr_gene_likelihood.txt'),
...
write.table(
   diplotypes_hrGenes, gzfile(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/diplotypes_hrGenes.txt.gz')),
   sep='\t',row.names=F,quote=F
)
...
write.table(
   diplotypes_hrd_hrGenes, gzfile(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/diplotypes_hrd_hrGenes.txt.gz')),
   sep='\t',row.names=F,quote=F
)

...
pred_hrd <- pred[pred$hrd >= 0.5,]

#--------- Get features used by chord ---------#
...
#--------- Fisher test ---------#
hr_gene_likelihood <- read.delim(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/hr_gene_likelihood.txt'))
...
genes_bed <- read.delim(paste0(base_dir,'/scripts_main/hmfGeneAnnotation/data/gene_selection/genes.bed'))
...
diplotypes_filt_ss <- diplotypes_filt[
  diplotypes_filt$ensembl_gene_id %in% hr_genes_extended
  & diplotypes_filt$sample %in% pred_hrd$sample
,]

#length(unique(diplotypes_filt_ss$ensembl_gene_id))

####################################################################################################
# Make diplotype matrix and clustering                                                             #
####################################################################################################

#========= Put data into wide format; make heatmap information layers =========#
mkDiplotypeMatrix <- function(
  ...
  if(output=='eff' | is.na(output) | is.null(output)){
    ...
    diplotypes_ss <- diplotypes[c(COMMON_COLS,'a1','a2')]
    ...
    if(boost.high.impact.eff.scores){
      diplotypes_ss[diplotypes_ss$a1=='full_gene_loss',c('a1.max_score','a2.max_score')] <- 5.4
      diplotypes_ss[diplotypes_ss$a1=='trunc',c('a1.max_score','a2.max_score')] <- 5.3
      
      diplotypes_ss[diplotypes_ss$a1=='loh','a1.max_score'] <- 5.2
      
      #diplotypes_ss[diplotypes_ss$a1=='frameshift_variant','a1.max_score'] <- 5.1
      diplotypes_ss[diplotypes_ss$a2=='frameshift_variant','a2.max_score'] <- 5.1
    }
    
    #diplotypes_ss[c('a1','a2')] <- NULL
...
## Export 
saveRDS(l_m_diplotypes, paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/l_m_diplotypes.rds'))
saveRDS(rank_order_clust, paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/data/rank_order_clust.rds'))


library(openxlsx)

formatLMDiplotypes <- function(l_m_diplotypes){
  lapply(l_m_diplotypes, function(i){
    i <- as.data.frame(i)
    sample_names <- analysis_sample_selection$hmf_id[ match(rownames(i), analysis_sample_selection$sample_id) ]
    cbind(index=1:nrow(i), sample=sample_names,cluster=rank_order_clust$clusters,i)
  })
}

l_m_diplotypes_export <- formatLMDiplotypes(l_m_diplotypes)

#write.xlsx(l_m_diplotypes_export, paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/supp_tables/l_m_diplotypes.xlsx'))
...
write.xlsx(l_m_diplotypes_extended_export, paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/supp_tables/l_m_diplotypes_extended.xlsx'))
...
plotDiplotypeHeatmap <- function(
  ...
  eff.metadata=read.delim(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/scripts/eff_metadata.txt')),
  auto.color='#c2c2c2',
  title=NULL
){
...
# png_height <- ncol(l_m_diplotypes_extended$eff)/10
# png(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/overview_hrd_samples_extended.png'),10, png_height,'in', res=1000)
# grid.draw(p)
# dev.off()
...
png(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/overview_hrd_samples.png'),10, 4.5,'in', res=1000)
...
png(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/overview_hrd_samples_extended.png'),10, 7,'in', res=600)
...
metadata_ss <- metadata[match(names(rank_order_clust$clusters), metadata$sample_id),c('sample_id','primary_tumor_location')]

cancer_types_sel <- c('Ovary','Pancreas','Breast','Prostate','Biliary','Urinary tract')
sample_groups <- lapply(cancer_types_sel, function(i){
  metadata_ss[metadata_ss$primary_tumor_location==i,'sample_id']
}); names(sample_groups) <- cancer_types_sel

sample_groups[['Other']] <- metadata_ss[!(metadata_ss$primary_tumor_location %in% cancer_types_sel),'sample_id']
...
png(paste0(base_dir,'/HMF_DR010_DR047/analysis/pancancer_overview/plots/overview_hrd_samples_by_cancer_type.png'),10, 5,'in', res=1000)

...
The vcf.* and df.* arguments can be mixed.
```
contexts <- extractSigsChord(
  vcf.snv = /path/to/vcf/with/snvs/,
  vcf.indel = /path/to/vcf/with/snvs/,
...
```
This is especially useful in the case of SV vcfs, since it is possible that you will need to parse 
these yourself.
...
## 2. Running CHORD by supplying dataframes
...
For SNVs and indels, dataframe containing CHROM, POS, REF and ALT information (i.e. a bed file like 
...
both SNVs and indels. This is not a problem as ```extractSigsChord()```
```{r message=FALSE, warning=FALSE}
df_snv_indel[25:30,]
```

...
To extract the confidence interval estimates:
```{r message=FALSE, warning=FALSE}
chord_output$ci_estimates
```

The confidence intervals (CI) give an idea of the likely error of each prediction probability (i.e.
errorbar upper and lower values). 

CI estimates are determined by resampling the feature vector for each sample 20 times and
calculating HRD probabilities for each iteration. The probabilities at the 5% and 95% quantiles are
then calculated.
